...
app.layout = html.Div(children=[
    ...
    html.Div([dcc.Upload(
        ...
        multiple=True), 
        html.Button("Continue labelling",
        id = "continue",
        style={
            'display': 'inline-block',
            'margin': '10px'
    ...
    html.Div([
        html.Div([
            html.Button(
                    children=[html.Img(src=app.get_asset_url('arrow-left.png'),
                                       style={"height": "25px",
                                              "width": "25px"}
                                       ), ],
        ...
        html.Div([
            html.Button('Mark all as clean', id='clean_all',
                        style={'margin': '10px', 'height': '40px'},
                        n_clicks=0),
            html.Button('Mark all as artifact', id='artifact_all',
                        style={'margin': '10px', 'height': '40px'},
                        n_clicks=0)], style={"display": "inline-block"}),
        html.Div([
               html.Button(
                    children=[html.Img(src=app.get_asset_url('arrow-right.png'),
                                       style={"height": "25px",
                                              "width": "25px"}
                                       ), ],
    ...
    html.Div([
        html.Button(
            ...
            style={'padding': '5px', "height": "40px",'display':'inline-block','float':'left'},
            id = "prev-epoch",
            n_clicks = 0,),
        html.Button(
            children=[html.Img(src=app.get_asset_url('arrow-right.png'),
                    style={"height": "25px","width": "25px"}), ],
            style={'padding': '5px', "height": "40px",'display':'inline-block','float':'right'},
            id = "next-epoch",
            n_clicks = 0,),       
    ]),
    html.Br(),
    ...
    html.Div([
        html.Div([
            html.Label('Label:',style={'float':'left','margin-left':'150px'}),
            dcc.RadioItems(
                options=[
            ...
            dcc.RadioItems(
                ...
                labelStyle={'display': 'inline-block'},
                style={'float':'left'}),
        ],style={'margin':'0 auto','width':'60%', 'text-align':'center','margin-bottom':'15px'}),
        html.Br(),
        html.Div([
            ...
            dcc.RadioItems(
                options=[
                    ...
                    {'label':'Changes quickly', 'value':'changes_quickly'},
                    {'label':'Rises quickly', 'value':'rise_quickly'},
                    {'label':'Drops quickly', 'value':'drops_quickly'},
                    {'label':'Invalid portion - left', 'value':'invalid-left'},
                    {'label':'Invalid portion - right', 'value':'invalid-right'}],
                    value = 'none',
                    id = 'artifact-type',
                    labelStyle = {'display':'inline-block'},
        ...
        ],style={'margin':'0 auto','width':'20%', 'text-align':'center','margin-bottom':'15px'}),
...
@app.callback(Output(component_id='eda_parts', component_property='figure'),
...
@app.callback(Output(component_id='acc_data',component_property='figure'),
              [Input('eda_parts', 'figure')])
...
@app.callback(Output(component_id='detailed_view', component_property='figure'),
            [Input('eda_parts', 'clickData'),
            Input('done', 'n_clicks'),
            Input('eda_parts','figure')])
...
@app.callback(Output(component_id='artifact-labelling', component_property='style'),
              [Input('eda_parts', 'figure')])
...
@app.callback(Output(component_id='label', component_property='value'),
              [Input('eda_parts', 'figure')])
...
@app.callback(Output(component_id='confidence', component_property='value'),
              [Input('eda_parts', 'figure')])
...
@app.callback(Output(component_id='artifact-type', component_property='value'),
              [Input('eda_parts', 'figure')])
