...
class EDAData:
    ...
    def reset(self):
        ...
        self.current_epoch_left, self.current_epoch_right, self.current_epoch_label = (
            None,
            None,
            None,
        )
    ...
    def setup(self):
        ...
        self.max_epoch = 1
        self.progress = 1
        self.current_window_left, self.current_window_right = self.get_window_margin(
            self.current_window
        )
        (
            self.current_epoch_left,
            self.current_epoch_right,
            self.current_epoch_label,
        ) = self.get_interval(self.current_epoch)
    ...
    def upload(self, vals, time):
        if self.loaded == False:
            ...
            (
                self.current_window_left,
                self.current_window_right,
            ) = self.get_window_margin(self.current_window)
            (
                self.current_epoch_left,
                self.current_epoch_right,
                self.current_epoch_label,
    ...
    def get_interval_padded(self, interval):
        ...
        if interval - 1 >= 1:
            first = interval - 1
        left = self.eda[self.eda["intervals"] == first].head(1)["eda_Time"].values[0]
        right = self.eda[self.eda["intervals"] == last].tail(1)["eda_Time"].values[0]
    ...
    def get_window_margin(self, window):
        ...
        left = rows.head(1)["eda_Time"].values[0]
        right = rows.tail(1)["eda_Time"].values[0]
        return (str(left), str(right))
    ...
    def get_window_for_interval(self, interval):
        window = (
            self.eda[self.eda["intervals"] == interval].head(1)["windows"].values[0]
        )
        return window
    ...
    def get_progress(self):
        right = (
            self.eda[self.eda["windows"] == self.progress].tail(1)["eda_Time"].values[0]
        )
    ...
    def get_first_of_window(self):
        return (self.eda[self.eda["windows"] == self.current_window].head(1))[
            "intervals"
        ].values[0]
    ...
    def label_epoch(self, left, right, value, confidence, type_artifact):
        self.eda.loc[
            self.eda["eda_Time"].between(left, right, True),
            ["label", "confidence", "type"],
        ] = [value, confidence, type_artifact]
    ...
    def leave_window(self, window):
        self.eda.loc[
            (self.eda["windows"] == window) & (self.eda["label"] == "none"), "label"
        ] = "unsure"
    ...
    def set_current_epochs(self, epoch):
        if epoch <= self.max_epoch and epoch >= 1:
            if self.get_window_for_interval(epoch) != self.get_window_for_interval(
                self.current_epoch
            ):
